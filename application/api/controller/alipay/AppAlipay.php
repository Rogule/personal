...
class Alipay extends Controller
{
    ...
    private $Version = "1.0";
    private $format = "JSON";
    // private $NotifyUrl = "https://tapi.huike.jsojs.com/app/alipay/checkRsaSign";
...
//    private $Version = "1.0";
//    private $format = "JSON";
//    private $NotifyUrl = "http://api.huike.jsojs.com/common/alipay/checkRsaSign";
...
//            项目逻辑处理开始
                $this->output_log_file("order调用成功了");
...
//        $order = new Order();
        ...
        $whereStatus['orderNo'] = $param['out_trade_no'];
        $objOrder = db("order")->where($whereStatus)->find();
        //插入total_amount验证
        $this->output_log_file(json_encode($objOrder));
        if (!$objOrder)
            $this->output_result_alipay('5', '查无此单。');

        $total_amount = $objOrder['payableMoney'];
