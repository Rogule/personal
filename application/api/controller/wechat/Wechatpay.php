...
//            $retdata = $wxpay->GetJsApiParameters($order);
            if($retdata==null){
                return apiReturn(300,"支付类型不正确");
            }
            return apiReturn(200, "操作成功!",$retdata);

        } catch (\Exception $ex) {
            return apiReturn(300, $ex->getMessage());
        }
...
//        \Log::INFO($log);
        output_log_file(json_encode($log));
        //这里修改状态
...
//            $orderlog = WxpayLogModel::where('out_trade_no',$id)->find();
            output_log_file("+++++++++++++++++++++++++++");
            output_log_file(json_encode($orderlog));
            if($orderlog){
...
//                    查询商品下单数量
                    ...
                    output_log_file(json_encode($order));
//                    修改订单状态
